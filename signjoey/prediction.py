...
from signjoey.vocabulary import PAD_TOKEN, SIL_TOKEN, EOS_TOKEN
    ...
    with torch.no_grad():
        ...
        if do_translation:
            ...
            for tindx, t in enumerate(references):
                ...
                if t[-1].lower() == EOS_TOKEN(cfg_data, tokenizer):
                    references[tindx] = references[tindx][:-1]
...
def test(
    cfg_file, ckpt: str, output_path: str = None, logger: logging.Logger = None, tokenizer=None
) -> None:
    """
    ...
    # load the data
    _, dev_data, test_data, gls_vocab, txt_vocab, tokenizer = load_data(data_cfg=cfg["data"])
    ...
    if do_translation:
        translation_loss_function = XentLoss(
            pad_index=txt_vocab.stoi[PAD_TOKEN(cfg["data"], tokenizer)], smoothing=0.0
        )
        if use_cuda:
        ...
        for tbw in translation_beam_sizes:
            ...
            for ta in translation_beam_alphas:
                dev_translation_results[tbw][ta] = validate_on_data(
                    ...
                    translation_max_output_length=translation_max_output_length,
                    txt_pad_index=txt_vocab.stoi[PAD_TOKEN(cfg["data"], tokenizer)],
