...
def load_data(data_cfg: dict, keep_only = None) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    ...
    gls_field = data.Field(
        pad_token=PAD_TOKEN(data_cfg, tokenizer),
    ...
    txt_field = data.Field(
        init_token=BOS_TOKEN(data_cfg, tokenizer) if not mBartVocab else None,
        # For mBartVocab, 'de_DE' (in data file) is start token for training, and EOS for prediction.
        eos_token=EOS_TOKEN(data_cfg, tokenizer) if not mBartVocab else None,  # For mBartVocab, '</s>' is already included in the data file
        pad_token=PAD_TOKEN(data_cfg, tokenizer),
        tokenize=tokenize_text,
        unk_token=UNK_TOKEN(data_cfg, tokenizer),
        batch_first=True,
        lower=txt_lowercase,
        include_lengths=True,
    )
    ...
    txt_field_pred = data.Field(
        ...
        init_token=BOS_TOKEN(data_cfg, tokenizer) if not mBartVocab else EOS_TOKEN(data_cfg, tokenizer),  # EOS for prediction (config.decoder_start_token_id)
        eos_token=EOS_TOKEN(data_cfg, tokenizer) if not mBartVocab else None,  # For mBartVocab, '</s>' is already included in the data file
        pad_token=PAD_TOKEN(data_cfg, tokenizer),
        tokenize=tokenize_text,
        unk_token=UNK_TOKEN(data_cfg, tokenizer),
